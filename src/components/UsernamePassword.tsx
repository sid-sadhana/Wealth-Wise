...
import { set_signup_username } from '../redux/actions';
import { set_signup_password } from '../redux/actions';
...
const UsernamePassword: React.FC = () => {
  ...
  const usernameFromState = useSelector((state: RootState) => state.password);
  ...
  return (
    <div className="w-full">
      ...
      <button
        ...
        onClick={() => dispatch(set_signup_password(pass))}
      >
        Set Username
      </button>

      <h2>State Username: {usernameFromState}</h2>
    </div>
  );
};
